...
export class SessionManager {
	...
	async getSessionId(file: TFile): Promise<SessionResult> {
		...
		if (typeof storedId !== 'string' || storedId.length < 10) {
			new Notice('⚠️ Invalid Claude session ID in frontmatter. Starting new conversation.');
	...
	async handleInvalidSession(file: TFile): Promise<void> {
		new Notice('⚠️ Previous conversation could not be restored. Starting fresh.');
		await this.clearSession(file);
	}
}
